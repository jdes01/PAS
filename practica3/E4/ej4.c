...
int main() 
{
	...
	switch (fork()) 
	{
		...
		case 0: 
			...
			// CERRAMOS ESCRITURA DE LA TUBERIA 1
			...
			// CERRAMOS LECTURA EN LA TUBERIA 2
			...
			else if (nbytes == 0){ printf("[HIJO]: Detecto que mi padre ha cerrado la tuberia 1...\n"); }
			...
			else
			{
				...
				// CADA TOKEN UN NUMERO

				token   = strtok(buf,  ";"); //Leo el primer número
				token_2 = strtok(NULL, ";"); //Leo el segundo número

				for(int d=1; d <= atoi(token); d++){ if( atoi(token)%d == 0 ){ cont++; } }

	    		if(cont==2){ //El primer numero es primo
	    		 
	    			cont = 0;

	    			for(int d=1; d <= atoi(token_2); d++){ if( atoi(token_2)%d == 0 ){ cont++; } }

	    			if(cont==2){ //El segundo numero es primo 
	    			
	    				if(atoi(token_2) - atoi(token) == 2	){ sprintf(mensaje, "gemelos"); }

	    				else{ sprintf(mensaje, "primos"); }
	    			}

	    			else{ sprintf(mensaje, "no-primos"); }
	    		}

	    		else{ sprintf(mensaje, "no-primos"); }
			}

			// CERRAMOS LECTURA DE LA TUBERIA 1
		...
		default:
			...
			// CERRAMOS LA LECTURA EN LA TUBERIA 1
			...
			// CERRAMOS LA ESCRITURA EN LA TUBERIA 2
			...
			// METEMOS AMBOS NUMEROS EN EL BUFFER

			sprintf(buf, "%d;%d", num, num2); //mete ambos numeros en el buffer buf

			// ESCRIBIMOS EN LA TUBERIA 1
			...
			// CERRAMOS LA ESCRITURA EN LA TUBERIA 1
			...
			else if (nbytes == 0){ printf("[PADRE]: Detecto que mi hijo ha cerrado la tuberia 2...\n"); }

			else{ printf("[PADRE]: Leido %s de la tuberia 2.\n", mensaje); }
